'''
Found at http://ilan.schnell-web.net/prog/Vector3/Vector3.txt
...
def _xyzto012(c):
    if c in 'xyz':
        ...
        raise AttributeError(f"Vector3 instance has no attribute '{c}'")
...
def _args2tuple(funcname, args):
    ...
    if narg == 0:
        ...
        if len(data) != 3:
            raise TypeError(
                f'Vector3.{funcname}() takes sequence with 3 elements '
                '({len(data)} given),\n\t   when 1 argument is given')
    elif narg == 3:
        data = args
    else:
        raise TypeError(
            f'Vector3{funcname}() takes 0, 1 or 3 arguments ({narg} given)')
    assert len(data) == 3
    try:
        return tuple(map(float, data))
    except (TypeError, ValueError):
        raise TypeError(
            f"Vector3.{funcname}() can't convert elements to float")
...
class Vector3(numpy.ndarray):
    ...
    def __repr__(self):
        return 'Vector3' + repr(tuple(self))
    ...
    def cross(a, b):
        return Vector3(numpy.cross(a, b))
